---
name: configure-skilllord
description: Interactive installer for SkillLord — guides users through selecting and installing skills and rules to user-level or project-level directories, verifies paths, and optionally optimizes installed files.
---

# Configure SkillLord

An interactive, step-by-step installation wizard for the SkillLord project. Uses `AskUserQuestion` to guide users through selective installation of skills and rules, then verifies correctness and offers optimization.
...
- User wants to verify or fix an existing SkillLord installation
- User wants to optimize installed skills or rules for their project

## Prerequisites

This skill must be accessible to Claude Code before activation. Two ways to bootstrap:
1. **Via Plugin**: `/plugin install claude-skill-lord` — the plugin loads this skill automatically
2. **Manual**: Copy only this skill to `~/.claude/skills/configure-ecc/SKILL.md`, then activate by saying "configure ecc"

---

## Step 0: Clone SkillLord Repository

Before any installation, clone the latest SkillLord source to `/tmp`:

```bash
rm -rf /tmp/claude-skill-lord
git clone https://github.com/affaan-m/claude-skill-lord.git /tmp/claude-skill-lord
```

Set `SL_ROOT=/tmp/claude-skill-lord` as the source for all subsequent copy operations.

If the clone fails (network issues, etc.), use `AskUserQuestion` to ask the user to provide a local path to an existing SkillLord clone.
...
```
Question: "Where should SkillLord components be installed?"
Options:
...
```bash
cp -r $SL_ROOT/skills/<skill-name> $TARGET/skills/
```
...
cp -r $SL_ROOT/rules/common/* $TARGET/rules/

# Language-specific rules (flat copy into rules/)
cp -r $SL_ROOT/rules/typescript/* $TARGET/rules/   # if selected
cp -r $SL_ROOT/rules/python/* $TARGET/rules/        # if selected
cp -r $SL_ROOT/rules/golang/* $TARGET/rules/        # if selected
```
...
**Critical**: Only modify files in the installation target (`$TARGET/`), NEVER modify files in the source SkillLord repository (`$SL_ROOT/`).
...
```
## SkillLord Installation Complete
