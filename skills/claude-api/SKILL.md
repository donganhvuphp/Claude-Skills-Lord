...
> Pairs with: agentic-engineering, cost-aware-llm-pipeline
...
while True:
    ...
    # Execute tools, append tool_result messages, continue loop
```
...
|----------|---------|-------------|
| Prompt caching | Up to 90% on cached tokens | Repeated system prompts or large context |
| Batches API | 50% | Non-time-sensitive bulk processing |
| Haiku instead of Sonnet | ~75% | Simple tasks, classification, extraction |
| Shorter max_tokens | Variable | When output length is predictable |
